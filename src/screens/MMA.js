    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://cors-anywhere.herokuapp.com/https://api.smarkets.com/v3/events/?state=upcoming&type=mma_match&type_domain=mma&with_new_type=false&sort=id&limit=20&include_hidden=false"
        );
        const mmaEventsData = response.data.events;
        setMmaEvents(mmaEventsData);
        setLoading(false);
      } catch (error) {
